<x-layout title="Ninja Roster">
    <div class="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <a href="{{ route('ninjas.create') }}" class="inline-flex rounded-full bg-slate-700 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-cyan-500/10 transition hover:bg-slate-600">Create Ninja</a>
    </div>
    ...
    <ul class="grid gap-6 md:grid-cols-2">
        @foreach ($ninjas as $ninja)
            @if($ninja['skill'] != 'x')
                <li class="list-none">
                    <x-card href="{{ route('ninjas.show', $ninja['id']) }}" :highlight="$ninja['strength'] > 70">
                        <div class="space-y-3">
